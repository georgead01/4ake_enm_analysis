...
# Author: Jordi Cohen
#
# $Id: ruler.tcl,v 1.7 2023/10/16 17:21:06 jasonks2 Exp $
#
...
proc ::Ruler::draw_indicator {} {
  ...
  if {[winfo exists .neuron]} {
    set unit "um"
  } else {
    set unit "A"
  }
  graphics $ruler_mol color gray
  graphics $ruler_mol text "[expr -0.99*$display_width] [expr -0.97*$display_height] $display_front" "[format "%g" $ruler_unit]$unit" size 0.8
}
...
proc ::Ruler::draw_scale {} {
  ...
  graphics $ruler_mol text "[expr -0.9*$display_width] [expr -0.95*$display_height] $display_front" "[format "%g" $ruler_unit] $unit" size 1.0
