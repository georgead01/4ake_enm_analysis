...
#
# $Id: hbonds.tcl,v 1.11 2022/02/13 02:37:10 gumbart Exp $
#
...
#  } else { set multichain 1 }
  ...
  set chainlist [$sel1 get chain]
  if {[info exists sel2]} {
    set chainlist [join [lappend chainlist [$sel2 get chain]]]
  }
  if { [llength [lsort -unique $chainlist]] > 1 } {
    set multichain 1
  }
...
proc hbonds::hbonddetails {hbondlist} {
   ...
   set framehbond {}
      ...
      foreach { d } [lindex $hbondlist 0] { a } [lindex $hbondlist 1] {
          ...
          if { $multichain } { 
            if { [string compare [$donor get segname] "{}"] == 0 } { 
              lappend newhbond_donor [$donor get chain]
            } else {
              lappend newhbond_donor [$donor get segname] 
            }
          }
...
#             } else { lappend newhbond_donor "-$atomname" }
          ...
          if { $multichain } {
            if { [string compare [$acceptor get segname] "{}"] == 0 } { 
              lappend newhbond_acceptor [$acceptor get chain]
            } else {
              lappend newhbond_acceptor [$acceptor get segname] 
            }
          }
