##
## $Id: molefacture_internals.tcl,v 1.58 2023/11/02 15:14:39 dgomes Exp $
##
...
proc ::Molefacture::atom_picked_fctn { args } {
  ...
  if {[$sel get segname] == "CAP"} {
    tk_messageBox -message "Cannot Select or Edit Molecules' Anchor Atoms." \
...
proc ::Molefacture::apply_changes_to_parent_mol {} {
  ...
  if {$Molefacture::qwikmd == 1 } {
    ...
    ### return to qmtool
    if {$Molefacture::qmtool == 1 } {
      mol modstyle $repindex $newmol "CPK"
      #puts "Executing QMtool::molefacture_callback [molinfo top]"
      ::QMtool::molefacture_callback [molinfo top]
    }      
...
proc ::Molefacture::export_qmtool {} {
  
  #set answer [tk_messageBox -message "Exporting molecule to QMTool will close Molefacture. Do you want to continue?" -type yesno -title "Export to QMTool" -icon info -parent $Molefacture::topGui]
  #if {$answer == "no"} {return}
  # Create a new VMD molecule
  variable tmpmolid
  set sel [atomselect $tmpmolid "$Molefacture::notDumSel"]
  set newmol [mol fromsels $sel]
  set repindex [molinfo $newmol get numreps]
  mol addrep $newmol
  mol modselect $repindex $newmol "all"
  mol modstyle $repindex $newmol "CPK"
  mol modcolor $repindex $newmol Element
  # Load this molecule to QMtool
  ::QMtool::qmtool

  set ::QMtool::multiplicity 1
...
proc ::Molefacture::submitCGENFF { outpref } {
  ...
  if {$env(CGENFF) == "server" && $Molefacture::cgenffcaller} {
    ...
    ### Conditional test to be used whenever CGenFF server starts asking for the password
    # if {[eval "cgenffcaller callcgenff -molfiles $molfiles -username $env(CGENFFUSERNAME) \
    # -password $env(CGENFFPASSWORD)"] == 0}

    if {[eval "cgenffcaller callcgenff -molfiles $molfiles \
          -username $env(CGENFFUSERNAME)"] == 0} {

      set totalstr [cgenffcaller numstr]
      for {set i 0} {$i < $totalstr} {incr i} {
        set stream [cgenffcaller getstr $i]
      ...
      set Molefacture::cgenffsub [cgenffcaller getsubcount]
    } else {
    ...
    foreach mol $molfiles {
      ...
      ### Linux
      catch {exec $env(CGENFFPATH) -a $mol >& [file root $mol].str} 
